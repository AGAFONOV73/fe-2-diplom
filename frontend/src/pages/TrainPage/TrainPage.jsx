...
export function TrainPage() {
  ...
  const fromDraft =
    getBookingDraft()?.selectedTrain || getBookingDraft()?.train;
  ...
  const seatParamsKey = useMemo(
    () => JSON.stringify(seatParams ?? {}),
    [seatParams],
  );

  const returnParams = useMemo(() => {
    if (!hasReturn) return null;
    const { date_end, from_city_id, to_city_id, offset, ...rest } =
      searchParams || {};
  ...
  useEffect(() => {
    ...
    const fetchReturn = async () => {
      ...
      try {
        ...
        setReturnRoutes([]);
        setReturnError(
          e?.message || "Не удалось получить список обратных направлений",
        );
      } finally {
        if (returnRoutesInFlightRef.current !== requestId) return;
  ...
  useEffect(() => {
    ...
    const fetchReturnSeats = async () => {
      ...
      try {
        ...
        setReturnCoaches([]);
        setReturnError(
          e?.message || "Не удалось загрузить схему мест (обратно)",
        );
      } finally {
        if (returnSeatsInFlightRef.current !== requestId) return;
  ...
  const handleContinue = () => {
    const payload = {
      ...
      ...(hasReturn
        ? {
            returnTrain: returnTrain?.raw ?? returnTrain,
        ...
        <div className="train-page__layout">
          ...
          <div className="train-page__main">
            ...
            <div className="train-page__content">
              {!train ? (
                <div className="error">
                  Не удалось определить выбранное направление. Вернитесь к
                  поиску и выберите поезд заново.
                </div>
              ) : null}
              ...
              {hasReturn ? (
                <div className="train-page__return">
                  ...
                  {returnRoutesLoading || returnSeatsLoading ? (
                    <Loader />
                  ) : null}
                  {!returnRoutesLoading &&
                  !returnSeatsLoading &&
                  !returnError ? (
                    <>
                        ...
                        <div className="return-trains">
                          {(returnRoutes || []).slice(0, 5).map((r) => {
                            ...
                            return (
                              <div
                                key={mapped.id}
                                className="return-train-item"
                              >
                                <Button
                                  variant="outline"
                                  onClick={() => setReturnTrain(mapped)}
                                >
                                  Выбрать поезд №{mapped.number} {mapped.from} →{" "}
                                  {mapped.to}
                                </Button>
                              </div>
                            );
                          ...
                          {!returnRoutes?.length ? (
                            <div className="no-results">
                              Обратных поездов не найдено. Попробуйте изменить
                              дату.
                            </div>
                          ) : null}
                        </div>
                      ...
                      selectedSeats.length !== returnSelectedSeats.length ? (
                        <div className="error">
                          Для поездки туда‑обратно количество мест должно
                          совпадать.
                        </div>
                      ) : null}
                    </>
                ...
                <span className="total-price">
                  ...
                  {train
                    ? selectedSeats
                        .reduce(
                          (sum, s) =>
                            sum + (Number(s?.price ?? s?.basePrice ?? 0) || 0),
                          0,
                        )
                        .toLocaleString()
                    : "—"}{" "}
