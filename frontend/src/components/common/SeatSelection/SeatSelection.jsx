...
export function SeatSelection({
  ...
  const wagonTypeOptions = useMemo(
    () => [
      {
        key: "fourth",
        label: getCoachTypeLabel("fourth"),
        icon: getCoachTypeIcon("fourth"),
      },
      {
        key: "third",
        label: getCoachTypeLabel("third"),
        icon: getCoachTypeIcon("third"),
      },
      {
        key: "second",
        label: getCoachTypeLabel("second"),
        icon: getCoachTypeIcon("second"),
      },
      {
        key: "first",
        label: getCoachTypeLabel("first"),
        icon: getCoachTypeIcon("first"),
      },
    ],
    [],
  );
  ...
  const toggleService = (key, value) => {
    setSelectedSeats((prev) =>
      prev.map((s) => {
        ...
        const linensIncluded = !!coach?.is_linens_included;
        const includeWifi =
          typeof value?.include_wifi === "boolean"
            ? value.include_wifi
            : s.include_wifi;
        const includeLinensRaw =
          typeof value?.include_linens === "boolean"
            ? value.include_linens
            : s.include_linens;
        const includeLinens = linensIncluded ? true : includeLinensRaw;

        const wifiAdd = includeWifi ? Number(coach?.wifi_price ?? 0) : 0;
        const linensAdd =
          includeLinens && !linensIncluded
            ? Number(coach?.linens_price ?? 0)
            : 0;
  ...
  const getSeatsForScheme = (coachId) => {
    ...
    return getSeatsForCoach(coachId).filter(
      (s) => s.number >= 1 && s.number <= 32,
    );
  };
  ...
  const getSeatsForOverlay = (coachId) => {
    ...
    return list.filter(
      (s) => s.number >= 1 && s.number <= (Number.isFinite(max) ? max : 0),
    );
  };
  ...
  const schemeImage = useMemo(() => {
    if (coachType === "third" || coachType === "fourth")
      return sittingPlatzkartSchemeImg;
      ...
      <div className="ss-tickets-row">
        <div className="ss-ticket-card">
          ...
          <div className="ss-ticket-hint">
            Можно добавить еще {Math.max(0, maxSeats - tickets.adult)}{" "}
            пассажиров
          </div>
        </div>
        <div className="ss-ticket-card">
          ...
          <div className="ss-ticket-hint">
            Можно добавить еще {Math.max(0, maxSeats - tickets.child)}{" "}
            пассажиров
          </div>
        </div>
        <div className="ss-ticket-card">
      ...
      {filteredCoaches.length > 1 ? (
        <div className="ss-wagons-multi">
          {filteredCoaches.map((coach, idx) => {
            ...
            const canLinens =
              ...
              Number(coach?.linens_price ?? 0) > 0;
            const hasAc = Boolean(
              coach?.have_air_conditioning ??
              coach?.is_air_conditioning ??
              false,
            );
            ...
            return (
              <div key={coachId ?? idx} className="ss-wagon-section">
                  ...
                  <div className="ss-wagon-right">
                    <div className="ss-wagon-col ss-wagon-col--seats">
                      <div className="ss-wagon-seats-title">
                        Места{" "}
                        {coach?.avaliable_seats ?? coach?.available_seats ?? 0}
                      </div>
                      ...
                      {coachType === "third" || coachType === "second" ? (
                        <div className="ss-price-rows">
                          <div className="ss-price-row">
                            <span>
                              {coachType === "third" ? "Верхние" : "Верхние"}
                            </span>
                            <strong>{prices.top.toLocaleString()} ₽</strong>
                          </div>
                          <div className="ss-price-row">
                            <span>
                              {coachType === "third" ? "Нижние" : "Нижние"}
                            </span>
                    ...
                    <div className="ss-wagon-col ss-wagon-col--service">
                      ...
                      {hasAc ? (
                        <div className="service-text">кондиционер</div>
                      ) : null}
                    </div>
                  </div>
                </div>
                ...
                {schemeImage &&
                (coachType === "fourth" || coachType === "first") ? (
                  <div
        ...
        <>
          ...
          {(() => {
            ...
            const stats = coachId
              ? getSeatStatsForCoach(coachId, coachType)
              : { top: 0, bottom: 0, side: 0, sitting: 0 };
            ...
            <div className="selected-seats-list">
              {selectedSeats
                ...
                .map((s) => {
                  ...
                  const linensIncluded = !!coach?.is_linens_included;
                  const canLinens =
                    linensIncluded || Number(coach?.linens_price ?? 0) > 0;
                        ...
                        {canLinens ? (
                          <label>
                            <input
                              ...
                              onChange={(e) =>
                                toggleService(key, {
                                  include_linens: e.target.checked,
                                })
                              }
                            />
                            Бельё
